...
class TestWorkflowNotification(TestCase):
    ...
    def test_sends_resolved_webhook(self, safe_urlopen):
        ...
        assert faux(safe_urlopen).kwarg_equals('data.action', 'resolved')
        assert faux(safe_urlopen).kwarg_equals('headers.Sentry-Hook-Resource', 'issue')
        assert faux(safe_urlopen).kwarg_equals('data.data.id', six.binary_type(self.issue.id))
    ...
    def test_sends_resolved_webhook_as_Sentry_without_user(self, safe_urlopen):
        ...
        assert faux(safe_urlopen).kwarg_equals('data.actor.type', 'application')
        assert faux(safe_urlopen).kwarg_equals('data.actor.id', 'sentry')
        assert faux(safe_urlopen).kwarg_equals('data.actor.name', 'Sentry')
