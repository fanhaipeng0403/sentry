...
def issue_resolved_in_release(project, group, user, resolution_type, **kwargs):
    send_workflow_webhooks(project.organization, group, user, 'issue.resolved')
...
def resolved_with_commit(organization_id, group, user, **kwargs):
    ...
    send_workflow_webhooks(organization, group, user, 'issue.resolved')


def send_workflow_webhooks(organization, issue, user, event):
